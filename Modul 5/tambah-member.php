                    ...
                    <div class="row justify-content-center">
                        ...
                        <div class="col-6">
                            <form action="" method="post">
                                ...
                                <input type="text" name="nama_member" id="nama_member" class="form-control mb-3" value="<?php if (isset($_GET['id_member']))
                                                                                                                            echo $result[0]["nama_member"] ?>" required>

                                <label for="nomor_member" class="form-label">Nomor Member</label>
                                <input type="text" name="nomor_member" id="nomor_member" class="form-control mb-3" value="<?php if (isset($_GET['id_member']))
                                                                                                                                echo $result[0]["nomor_member"] ?>" required>

                                <label for="alamat" class="form-label">Alamat Member</label>
                                <textarea name="alamat" id="alamat" class="form-control" required><?php if (isset($_GET['id_member']))
                                                                                                        echo $result[0]["alamat"] ?> </textarea>

                                <label for="tgl_mendaftar" class="form-label">Tanggal Mendaftar</label>
                                <input type="datetime-local" name="tgl_mendaftar" id="tgl_mendaftar" class="form-control" value="<?php if (isset($_GET['id_member']))
                                                                                                                                        echo date('Y-m-d\TH:i', strtotime($result[0]["tgl_mendaftar"])) ?>" required>

                                <label for="tgl_terakhir_bayar" class="form-label">Tanggal Terakhir Bayar</label>
                                <input type="date" name="tgl_terakhir_bayar" id="tgl_terakhir_bayar" class="form-control" value="<?php if (isset($_GET['id_member']))
